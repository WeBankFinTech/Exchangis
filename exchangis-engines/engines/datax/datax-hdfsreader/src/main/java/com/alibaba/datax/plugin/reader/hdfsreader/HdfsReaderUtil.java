...
public class HdfsReaderUtil {
    ...
    HdfsReaderUtil(Configuration readerConfig) {
        ...
        this.haveKerberos = readerConfig.getBool(Key.HAVE_KERBEROS, false);
        HdfsUserGroupInfoLock.lock();
        try {
            if (haveKerberos) {
                this.kerberosKeytabFilePath = readerConfig.getString(Key.KERBEROS_KEYTAB_FILE_PATH);
                this.kerberosPrincipal = readerConfig.getString(Key.KERBEROS_PRINCIPAL);
                ...
                fileSystem = null == ugi? null : ugi.doAs((PrivilegedExceptionAction<FileSystem>) () -> {
                    ...
                    try {
                        ...
                        String message = String.format("获取FileSystem时发生网络IO异常,请检查您的网络是否正常!HDFS地址：[%s]",
                                "message:defaultFS =" + readerConfig.getString(Key.DEFAULT_FS));
                        LOG.error(message);
        ...
        try{
            ...
            try{
                ...
                if(StringUtils.isNotBlank(fileType) && !fileType
                        .equalsIgnoreCase(originConfig.getString(Key.FILETYPE, ""))){
                    affect = true;
                    originConfig.set(Key.FILETYPE, fileType);
                }
    ...
    private Hive getHiveConnByUris(String hiveMetaStoreUris) throws IOException, InterruptedException {
        ...
        if(haveKerberos){
            ...
            hiveConf.setVar(HiveConf.ConfVars.METASTORE_KERBEROS_PRINCIPAL,
                    this.readerConfig.getString(Key.HIVE_KEBEROS_PRINCIPAL, DEFAULT_HIVE_USER + "/_HOST@EXAMPLE.COM"));
