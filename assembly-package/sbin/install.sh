...
MAIN_MODULE="exchangis-server"
# Define the module path (main or embeded), the main module's path is the same as the home path default
MAIN_MODULE_PATH="${EXCHANGIS_HOME_PATH}"
EMBEDED_MODULE_PATH="${EXCHANGIS_HOME_PATH}/embeded"
...
usage(){
   ...
   printf "\t%-15s  %-15s  %-2s \n" "-m|--modules" "modules to install" "Define the modules to install, such as: [exchangis-server, embeded/eureka, embeded/spring-cloud-gateway]"
   ...
   printf "\t%-15s  %-15s  %-2s \n" "--unsafe" "unsafe mode" "Will clean the module's lib and config directory"
   printf "\t%-15s  %-15s  %-2s \n" "--safe" "safe mode" "Will not modify the module's lib and config directory(Default)"
