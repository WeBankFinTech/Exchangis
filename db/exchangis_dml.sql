...
,('setting.vld.vldNum','校验单号','','DATAX','INPUT','setting.vld.vldNum','校验单号','',1,'VARCHAR','','','REGEX','^[0-9]\\d*$','校验单号错误',0,0,'',1,'',7,'',1,NULL)
...
SELECT 'nullFormat','空值字符','DATAX-SOURCE','HIVE','INPUT','nullFormat','空值字符','',0,'VARCHAR','','','REGEX','^[\\s\\S]{0,50}$','空值字符输入错误',0,0,'',1,'',2,'',1,(SELECT id FROM `exchangis_job_param_config` WHERE config_key='transferMode' AND `type`='HIVE') FROM DUAL;
