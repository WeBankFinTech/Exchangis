...
if isWindows():
    ...
    CLASS_PATH = "%s/lib/*:." % DATAX_HOME
LOGBACK_FILE = "%s/conf/log/logback.xml" % DATAX_HOME
...
ENGINE_COMMAND = "%s/bin/java -server ${jvm} %s -classpath %s  " \
                 "${params} com.alibaba.datax.core.Engine -mode ${mode} -jobid ${jobid} -job ${job}" % (
                     JAVA_HOME, DEFAULT_PROPERTY_CONF, CLASS_PATH)
...
if __name__ == "__main__":
    ...
    # print startCommand
    child_process = subprocess.Popen(startCommand, shell=True, env=os.environ.copy())
    register_signal()
