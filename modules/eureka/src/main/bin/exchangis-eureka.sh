...
#
MAIN_CLASS=com.webank.wedatasphere.exchangis.EurekaApplication
FRIEND_NAME=EXCHANGIS-EUREKA
if [ ! ${ENV_FILE} ]; then
...
if [ ! ${EUREKA_LOG_PATH} ]; then
    EUREKA_LOG_PATH=${BIN}/../logs
fi

if [ ! ${EUREKA_CONF_PATH} ]; then
    EUREKA_CONF_PATH=${BIN}/../conf
fi
...
LIB_PATH=${BIN}/../lib
USER_DIR=${BIN}/../
CLASSPATH=${LIB_PATH}"/*:"${EUREKA_CONF_PATH}":."
...
JAVA_OPTS=${JAVA_OPTS}" -XX:HeapDumpPath="${EUREKA_LOG_PATH}" -Dlog.path="${EUREKA_LOG_PATH}
...
JAVA_OPTS=${JAVA_OPTS}" -Dlogging.config="${EUREKA_CONF_PATH}"/logback.xml"
JAVA_OPTS=${JAVA_OPTS}" -Deureka.hostname="${EUREKA_HOSTNAME}
