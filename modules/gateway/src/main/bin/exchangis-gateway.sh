...
#
MAIN_CLASS=com.webank.wedatasphere.exchangis.WebApplication
FRIEND_NAME=EXCHANGIS-GATEWAY
if [ ! ${ENV_FILE} ]; then
...
LIB_PATH=${BIN}/../lib
USER_DIR=${BIN}/../
CLASSPATH=${LIB_PATH}"/*:"${WEB_CONF_PATH}":."
...
JAVA_OPTS=${JAVA_OPTS}" -XX:HeapDumpPath="${WEB_LOG_PATH}" -Dlog.path="${WEB_LOG_PATH}
...
JAVA_OPTS=${JAVA_OPTS}" -Dlogging.config="${WEB_CONF_PATH}"/logback.xml"
