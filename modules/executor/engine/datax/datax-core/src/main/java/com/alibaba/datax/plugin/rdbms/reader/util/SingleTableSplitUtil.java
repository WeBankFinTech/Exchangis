...
public class SingleTableSplitUtil {
    ...
    public static List<Configuration> splitSingleTable(
        ...
        if (DATABASE_TYPE == DataBaseType.Oracle) {
            ...
            boolean isStringType = com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE_STRING.equals(configuration
                    .getString(com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE));
            boolean isLongType = com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE_LONG.equals(configuration
                    .getString(com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE));
    ...
    public static String buildQuerySql(String column, String table,
        ...
        if (StringUtils.isBlank(where)) {
            querySql = String.format(com.alibaba.datax.plugin.rdbms.reader.Constant.QUERY_SQL_TEMPLATE_WITHOUT_WHERE,
                    column, table);
        } else {
            querySql = String.format(com.alibaba.datax.plugin.rdbms.reader.Constant.QUERY_SQL_TEMPLATE, column,
                    table, where);
        }
    ...
    private static Pair<Object, Object> getPkRange(Configuration configuration) {
        ...
        int fetchSize = configuration.getInt(com.alibaba.datax.plugin.rdbms.reader.Constant.FETCH_SIZE);
            ...
            if (isPKTypeValid(rsMetaData)) {
                if (isStringType(rsMetaData.getColumnType(1))) {
                    if (configuration != null) {
                        configuration
                                .set(com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE, com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE_STRING);
                    }
                    ...
                    if (configuration != null) {
                        configuration.set(com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE, com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE_LONG);
                    }
    ...
    public static List<String> genSplitSqlForOracle(String splitPK,
        ...
        int fetchSize = configuration.getInt(com.alibaba.datax.plugin.rdbms.reader.Constant.FETCH_SIZE, 32);
        String jdbcURL = configuration.getString(Key.JDBC_URL);
        String username = configuration.getString(Key.USERNAME);
        String password = configuration.getString(Key.PASSWORD);
        ...
        try {
            ...
            if (configuration != null) {
                configuration
                        .set(com.alibaba.datax.plugin.rdbms.reader.Constant.PK_TYPE, Constant.PK_TYPE_MONTECARLO);
            }
            ResultSetMetaData rsMetaData = rs.getMetaData();
            while (DBUtil.asyncResultSetNext(rs)) {
